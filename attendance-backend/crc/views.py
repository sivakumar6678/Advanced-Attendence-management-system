...
class StudentUpgradeView(APIView):
    def post(self, request):
        ...
        students_to_upgrade = Student.objects.filter(
            ...
            semester=semester,
            academic_year=academic_year,
            is_graduated=False  # 🚫 avoid upgrading already graduated students
        )
        ...
        for student in students_to_upgrade:
            # ✅ Upgrade Logic
            if student.year == 4 and student.semester == 2:
                # Final year second semester -> Graduated
                student.is_graduated = True
            else:
                # Upgrade semester and possibly year
